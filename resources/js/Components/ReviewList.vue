    ...
    <h2 class="text-3xl text-slate-900 dark:text-slate-100">Tenant Reviews of {{ name }}</h2>
        <template v-if="reviewsCount > 0">
            ...
            <div class="mt-8 flex flex-col sm:flex-row space-y-6 sm:space-y-0 sm:space-x-4 md:space-x-8">
                ...
                <div class="bg-slate-200 overflow-hidden rounded-md flex-1">
                            ...
                            <div class="flex-1">
                                ...
                                <template v-else>
                                    ...
                                    <div class="hidden md:inline-flex md:flex-col md:mt-4 md:w-full xl:flex-row">
                                        ...
                                        <Link :href="route('login')" class="mt-3 xl:mt-0 xl:ml-3 inline-flex items-center justify-center w-full md:w-auto px-6 py-3 border border-slate-300 text-base font-medium rounded-md shadow-sm text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">Login</Link>
                                    </div>
                                </template>
                            </div>
                        </div>
        ...
        <template v-else>
            <!-- No Reviews -->
            <div class="mt-8 bg-slate-900 border border-slate-600 overflow-hidden rounded-xl flex-1">
                <div class="px-4 py-5 flex flex-col md:flex-row items-center justify-center space-x-10 sm:p-6">
                    <div class="flex-1">
            ...
            <Dialog as="div" class="fixed z-10 inset-0 overflow-y-auto" @close="open = false">
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                        <div class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-xl px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full sm:p-6">
                            <div class="sm:flex sm:items-start">
                                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-sky-100 dark:bg-sky-700 dark:bg-opacity-40 sm:mx-0 sm:h-10 sm:w-10">
                                ...
                                <div class="mt-3 w-full text-center sm:mt-0 sm:ml-6 sm:text-left">
                                    <DialogTitle as="h3" class="text-lg leading-6 font-medium text-slate-900 dark:text-slate-200">
                                                ...
                                                <RadioGroupOption as="template" v-for="rating in ratings" :key="rating.name" :value="rating" v-slot="{ active, checked }">
                                                    <div
                                                        ...
                                                        :class="[
                                                            ...
                                                            checked ? 'bg-sky-600 border-transparent text-white hover:bg-sky-700' : 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-500 text-slate-900 hover:bg-slate-50 dark:hover:bg-slate-600',
                                                            'cursor-pointer focus:outline-none border rounded-md py-3 px-3 flex flex-col items-center justify-center text-sm font-medium uppercase sm:flex-1'
                                                        ]"
                                                    >
                                                ...
                                                <RadioGroupOption as="template" v-for="tenantOption in tenant" :key="tenantOption.name" :value="tenantOption" v-slot="{ active, checked }">
                                                    <div
                                                        ...
                                                        :class="[
                                                            active ? 'ring-2 ring-sky-500' : '',
                                                            checked ? 'bg-sky-600 border-transparent text-white hover:bg-sky-700' : 'bg-white dark:bg-slate-700 border-slate-200 dark:border-slate-500 text-slate-900 hover:bg-slate-50 dark:hover:bg-slate-600',
                                                            'cursor-pointer focus:outline-none border rounded-md py-2 px-2 flex flex-col items-center justify-center text-sm font-medium uppercase sm:flex-1'
                                                        ]"
                                                    >
                                                        <RadioGroupLabel as="p" class="inline-flex items-center text-base dark:text-slate-200">
                                                            {{ tenantOption.name }}
